...
public class GameController
{
    ...
    public void StartGame()
    {
        ...
        {
            _display.DrawGrid(_grid);
            _display.DrawScore(_scoreKeeper.totalScore);
            _display.DrawMessage("Enter move (row1 col1 row2 col2) or 'q' to quit:");
            ...
            if (parts.Length != 4)
            {
                _display.DrawMessage("Invalid input — enter four numbers separated by spaces.");
                continue;
            }
            ...
            {
                _display.DrawMessage("Invalid input — all four values must be integers.");
                continue;
            }

            if (!_validator.IsValidSwap(r1, c1, r2, c2))
            {
                _display.DrawMessage("Invalid swap — gems must be adjacent (no diagonals).");
                continue;
            }
            ...
            if (matches.Count == 0)
            {
                ...
                _display.DrawMessage("No matches formed — swap undone.");
            }
            else
            {
                ProcessMatches();
        ...
        _display.DrawMessage($"Game over! Final score: {_scoreKeeper.totalScore}");
    }
